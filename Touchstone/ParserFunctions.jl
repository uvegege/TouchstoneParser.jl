...
function get_networkdata(ps, ts, line, f)
    ...
    while !eof(f) & !is_noisedata(line)
        ...
        if !f_comment(ps, ts, line, f) & !isempty(line) # Check if is a comment
            ...
            # In some rare cases maybe you don't know the number of ports. Imagine a s2p without extension.
            if n_ports == 0
            ...
            # When reading a new line load frequency value
            if is_newline
                is_newline = false
                freq = values_float[1]
...
const f_comment(ps, ts, line, f) = checkpart!(is_comment, get_comment, ps, ts, line, f)
const f_optionline(ps, ts, line, f) = checkpart!(is_optionline, get_optionline, ps, ts, line, f)
const f_version(ps, ts, line, f) = checkpart!(is_version, get_version, ps, ts, line, f)
const f_nports(ps, ts, line, f) = checkpart!(is_nports, get_nports, ps, ts, line, f)
const f_matrixformat(ps, ts, line, f) = checkpart!(is_matrixformat, get_matrixformat, ps, ts, line, f)
const f_twoport_order(ps, ts, line, f) = checkpart!(is_twoport_order, get_twoport_order, ps, ts, line, f)
const f_n_freqs(ps, ts, line, f) = checkpart!(is_n_freqs, get_n_freqs, ps, ts, line, f)
const f_noise_freqs(ps, ts, line, f) = checkpart!(is_noise_freqs, get_noise_freqs, ps, ts, line, f)
const f_reference(ps, ts, line, f) = checkpart!(is_reference, get_reference, ps, ts, line, f)
const f_mixedmode_order(ps, ts, line, f) = checkpart!(is_mixedmode_order, get_mixedmode_order, ps, ts, line, f)
const f_info(ps, ts, line, f) = checkpart!(is_begininfo, get_info, ps, ts, line, f)
const f_networkdata(ps, ts, line, f) = checkpart!(is_networkdata, get_networkdata, ps, ts, line, f)
const f_noisedata(ps, ts, line, f) = checkpart!(is_noisedata, get_noisedata, ps, ts, line, f)
const f_endfile(ps, ts, line, f) = checkpart!(is_endfile, get_endfile, ps, ts, line, f)
const f_hfss_gamma(ps, ts, line, f) = checkpart!(is_hfss_gamma, get_hfss_gamma, ps, ts, line, f)
const f_hfss_portimpedance(ps, ts, line, f) = checkpart!(is_hfss_portimpedance, get_hfss_portimpedance, ps, ts, line, f)
const f_hfss_port(ps, ts, line, f) = checkpart!(is_hfss_port, get_hfss_port, ps, ts, line, f)
const f_hfss_modaldata(ps, ts, line, f) = checkpart!(is_hfss_modaldata, get_hfss_modaldata, ps, ts, line, f)
